...
백업이 VMware vSphere 웹 클라이언트 GUI에 마운트된 것으로 나열되지만 백업 마운트 해제 화면에 표시되지 않는 경우 REST API를 사용해야 합니다 `/backup/{backup-Id}/cleanup` 바인딩된 데이터 저장소를 정리하려면 마운트 해제 절차를 다시 시도하십시오.
